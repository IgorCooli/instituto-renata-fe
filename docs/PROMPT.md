...
**Layout área logada:** o **início** (`/app` index) **não** tem navbar superior nem lateral — apenas **`ThemeToggleScreenCorner`** e o conteúdo do dashboard. As **telas de módulo** (`/app/marketing`, etc.) usam **`AppFeatureShell`**: sidebar lateral **recolhível** (botão na topbar; em mobile painel sobreposto + backdrop e atalho circular no canto quando fechada), título do módulo, links em `featureSidebar.ts`, itens “Em breve” desativados; barra superior com **← Voltar** para `/app`, **tema** e avatar; rodapé da sidebar com perfil e **Sair**.
